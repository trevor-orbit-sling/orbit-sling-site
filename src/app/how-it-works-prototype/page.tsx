...
const HOW_IT_WORKS_CONTENT = {
  hero: {
    ...
    copy:
      "Our process is simple: diagnose the leaks, build the right workflows, validate what works in market, and scale the system with clean reporting.",
  },
  engagement: {
    eyebrow: "Stage 1",
  ...
  flowPrototype: {
    eyebrow: "Visual Flow",
    title: "How the system moves from diagnosis to scale",
    intro:
      "This visual maps the same process below: diagnose the leaks, design the right system, launch the workflows, and keep improving with clean reporting.",
  },
  cta: {
...
export const metadata: Metadata = {
  title: { absolute: "How We Build Human-Led AI Sales Systems | Visual Flow Prototype" },
  description:
    "Prototype of Orbit Sling's human-led AI sales system process, including a visual flow from diagnosis to scaled execution.",
};
  ...
  return (
    <div className="page-shell team-shell">
      ...
      <main className="container content-main team-main">
        ...
        <section className="section" aria-label="Visual process flow prototype">
          <span className="eyebrow">{content.flowPrototype.eyebrow}</span>
          <h2 className="section-title">{content.flowPrototype.title}</h2>
          <p className="muted flow-prototype-intro">{content.flowPrototype.intro}</p>

          <div className="flow-prototype-shell">
            <div className="flow-prototype-track">
              {content.process.steps.map((step, index) => (
                <article key={`flow-${step.label}`} className="flow-prototype-node">
                  <p className="flow-prototype-step">{step.label}</p>
                  <h3 className="flow-prototype-title">{step.goal}</h3>
                  <p className="muted">{step.text}</p>
                  <p className="flow-prototype-deliverable">{step.deliverable}</p>
                  {index === 1 ? (
                    <>
                      <span className="flow-prototype-wrap-segment flow-prototype-wrap-segment-out" aria-hidden="true" />
                      <span className="flow-prototype-wrap-segment flow-prototype-wrap-segment-down-right" aria-hidden="true" />
                      <span className="flow-prototype-wrap-segment flow-prototype-wrap-segment-across" aria-hidden="true" />
                      <span className="flow-prototype-wrap-segment flow-prototype-wrap-segment-down-left" aria-hidden="true" />
                      <span className="flow-prototype-wrap-segment flow-prototype-wrap-segment-in" aria-hidden="true" />
                      <span className="flow-prototype-wrap-arrow" aria-hidden="true" />
                    </>
                  ) : null}
                </article>
              ))}
